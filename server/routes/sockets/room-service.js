...
class RoomService {
  ...
  onConnection(socket) {
    ...
    socket.on('join_room', (eventInfo) =>
      this.clientJoinRoomEvent(eventInfo, socket)
    )
    // Handle a user voting on an option.
    socket.on('user_vote', (eventInfo) =>
      this.clientVoteEvent(eventInfo, socket)
    )
    // Handle a user cancelling their vote.
    socket.on('user_cancel_vote', (eventInfo) =>
      this.clientCancelVoteEvent(eventInfo, socket)
    )
    // Handle a user creating a room. (Host)
    socket.on('create_room', (eventInfo) =>
      this.clientStartRoomEvent(eventInfo, socket)
    )
    // Handle a user closing a room. (Host)
    socket.on('close_room', (eventInfo) =>
      this.clientCloseRoomEvent(eventInfo, socket)
    )
    // Handle a user kicking another user from a room. (Host)
    socket.on('kick_user', (eventInfo) =>
      this.clientKickUserEvent(eventInfo, socket)
    )
    // Handle a user starting a new round of voting. (Host)
    socket.on('start_new_round', (eventInfo) =>
      this.clientStartNewRoundEvent(eventInfo, socket)
    )
    // Handle a user forcing the round to be over. (Host)
    socket.on('force_end_bidding', (eventInfo) =>
      this.clientForceEndBidding(eventInfo, socket)
    )
  }
  ...
  onDisconnect(socket) {
    try {
      ...
      if (connectedRoomID && this.activeRoomsByID[connectedRoomID]) {
  ...
  clientJoinRoomEvent(eventInfo, socket) {
    if (!eventInfo.roomID || !eventInfo.nickname) {
  ...
  clientStartRoomEvent(eventInfo, socket) {
    if (!eventInfo.roomID || !eventInfo.roomConfig) {
  ...
  clientKickUserEvent(eventInfo, socket) {
    if (!eventInfo.roomID || !eventInfo.user) {
  ...
  clientVoteEvent(eventInfo, socket) {
    if (!eventInfo.roomID || eventInfo.cardIndex === null) {
