...
export function onRoomStateChanged(socket, callback) {
    return registerOn(
        socket, 
        Events.ROOM_STATE_CHANGED,
        (event) => callback(event.roomState),
    )
}
...
export function onHostRoomClosedSuccess(socket, callback) {
    return registerOn(
        socket, 
        Events.HOST_ROOM_CLOSED_SUCCESS,
        (_) => callback(),
    )
}
